...
public abstract class RAAbstractArchitectCamActivity extends Activity implements ArchitectViewHolderInterface{
			...
			this.locationListener = new LocationListener() {
				...
				public void onLocationChanged( final Location location ) {
					...
					// sore last location as member, in case it is needed somewhere (in e.g. your adjusted project)
						RAAbstractArchitectCamActivity.this.lastKnownLocaton = location;
						if ( RAAbstractArchitectCamActivity.this.architectView != null ) {
							// check if location has altitude at certain accuracy level & call right architect method (the one with altitude information)
							if ( location.hasAltitude() && location.hasAccuracy() && location.getAccuracy()<7) {
								RAAbstractArchitectCamActivity.this.architectView.setLocation( location.getLatitude(), location.getLongitude(), location.getAltitude(), location.getAccuracy() );
							} else {
								RAAbstractArchitectCamActivity.this.architectView.setLocation( location.getLatitude(), location.getLongitude(), location.hasAccuracy() ? location.getAccuracy() : 1000 );
							}
						}
					}
				}
				...
				public void run() {
					...
					while (lastKnownLocaton==null && !isFinishing()) {
						...
						runOnUiThread(new Runnable() {
							...
							@Override
							public void run() {
								Toast.makeText(RAAbstractArchitectCamActivity.this, "Trying to find out where you are", Toast.LENGTH_SHORT).show();	
							}
						});
